...
type Post struct {
	...
	Commentslist []uint64 `protobuf:"varint,5,rep,packed,name=commentslist,proto3" json:"commentslist,omitempty"`
}
...
func (m *Post) GetCommentslist() []uint64 {
	if m != nil {
...
var fileDescriptor_1da4a141e1c534d3 = []byte{
	...
	0xce, 0xcd, 0xcf, 0x4b, 0x2c, 0x2d, 0xd1, 0x03, 0x49, 0x81, 0x09, 0xa5, 0x3a, 0x2e, 0x96, 0x80,
	...
	0x0b, 0x82, 0xd9, 0x42, 0x4a, 0x5c, 0x3c, 0xc9, 0xf9, 0xb9, 0xb9, 0xa9, 0x79, 0x25, 0xc5, 0x39,
	0x99, 0xc5, 0x25, 0x12, 0xac, 0x0a, 0xcc, 0x1a, 0x2c, 0x41, 0x28, 0x62, 0x4e, 0x0e, 0x27, 0x1e,
	...
	0x97, 0x9c, 0x9f, 0xab, 0x0f, 0x77, 0xb9, 0x3e, 0xd8, 0x53, 0x15, 0x10, 0xaa, 0xa4, 0xb2, 0x20,
	0xb5, 0x38, 0x89, 0x0d, 0xec, 0x3b, 0x63, 0x40, 0x00, 0x00, 0x00, 0xff, 0xff, 0x1c, 0x5c, 0xed,
	0x7d, 0xf0, 0x00, 0x00, 0x00,
}
...
func (m *Post) MarshalToSizedBuffer(dAtA []byte) (int, error) {
	...
	if len(m.Commentslist) > 0 {
		dAtA2 := make([]byte, len(m.Commentslist)*10)
		var j1 int
		for _, num := range m.Commentslist {
			for num >= 1<<7 {
	...
	for iNdEx < l {
		...
		case 5:
			if wireType == 0 {
				...
				m.Commentslist = append(m.Commentslist, v)
			} else if wireType == 2 {
				var packedLen int
				for shift := uint(0); ; shift += 7 {
					if shift >= 64 {
				...
				elementCount = count
				if elementCount != 0 && len(m.Commentslist) == 0 {
					m.Commentslist = make([]uint64, 0, elementCount)
				}
				for iNdEx < postIndex {
					var v uint64
					for shift := uint(0); ; shift += 7 {
