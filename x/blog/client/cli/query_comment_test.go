...
func networkWithCommentObjects(t *testing.T, n int) (*network.Network, []types.Comment) {
	...
	for i := 0; i < n; i++ {
		state.CommentList = append(state.CommentList, types.Comment{
			Id: uint64(i),
		})
	}
	...
	for _, tc := range []struct {
		...
		t.Run(tc.desc, func(t *testing.T) {
			...
			if tc.err != nil {
				...
				require.Equal(t, tc.obj, resp.Comment)
			}
		})
	}
}
...
func TestListComment(t *testing.T) {
	...
	t.Run("ByOffset", func(t *testing.T) {
		...
		for i := 0; i < len(objs); i += step {
			...
			require.Subset(t, objs, resp.Comment)
		}
	})
	...
	t.Run("Total", func(t *testing.T) {
		...
		require.Equal(t, objs, resp.Comment)
	})
}
